...
(async () => {
  ...
  const authCookie = JSON.parse(
    fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.15-r1/auth-cookie.json`, 'utf8')
  );
  ...
  const courseId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.15-r1/course-id.txt`, 'utf8').trim();
  const lessonOneId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.15-r1/lesson-one-id.txt`, 'utf8').trim();
  const lessonTwoId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.15-r1/lesson-two-id.txt`, 'utf8').trim();
