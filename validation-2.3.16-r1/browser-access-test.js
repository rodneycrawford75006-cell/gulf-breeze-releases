...
(async () => {
  ...
  const authCookie = JSON.parse(
    fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r1/auth-cookie.json`, 'utf8')
  );
  ...
  const courseId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r1/course-id.txt`, 'utf8').trim();
  const lessonOneId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r1/lesson-one-id.txt`, 'utf8').trim();
  const lessonTwoId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r1/lesson-two-id.txt`, 'utf8').trim();
  ...
  if (await studentPage.getByText('Leave a Reply', { exact: true }).count()) {
    throw new Error('Course page exposed a public comment form.');
  }
  if (await studentPage.getByText(/Internal Gulf Breeze POI/i).count()) {
    throw new Error('Course page exposed an internal curriculum description.');
  }
  ...
  if (await studentPage.getByText('Leave a Reply', { exact: true }).count()) {
    throw new Error('Lesson page exposed a public comment form.');
  }
  if (!(await studentPage.getByText(/Private self-check — no submission required:/i).count())) {
    throw new Error('Lesson page did not explain that the self-check is private and requires no submission.');
  }
