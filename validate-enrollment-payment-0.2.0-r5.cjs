...
const root = process.env.GB_EP_PLUGIN_DIR || path.resolve(__dirname, 'work_r5', 'gulf-breeze-enrollment-payment');
...
const checks = [
  ['version r5', /Version:\s*0\.2\.0-dev-r5/.test(php) && /const VERSION = '0\.2\.0-dev-r5'/.test(php)],
  ...
  ['no passwordless auth-cookie login', !php.includes('wp_set_auth_cookie') && !php.includes('wc_set_customer_auth_cookie')],
  ...
  ['no stale development version', !/0\.2\.0-dev-r[1-4]/.test(php + readme)],
];
