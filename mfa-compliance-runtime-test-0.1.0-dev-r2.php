...
gb_mfa_test_assert( Gulf_Breeze_MFA_Compliance::VERSION === '0.1.0-dev-r2', 'expected plugin version active' );
...
gb_mfa_test_assert( ! $plugin->student_requires_mfa( $owner->ID ), 'owner immunity overrides every student requirement source' );
...
gb_mfa_test_assert( false !== strpos( $redirect, '/mfa-enroll/' ), 'unenrolled student is directed to MFA enrollment immediately after login' );

$course_student_id = wp_create_user( 'validation-course-student', 'validation-only-password', 'validation-course-student@example.invalid' );
gb_mfa_test_assert( ! is_wp_error( $course_student_id ), 'course-enrolled student fixture created' );
$course_id = wp_insert_post( array( 'post_type' => 'lp_course', 'post_status' => 'publish', 'post_title' => 'Validation Course' ) );
$lp_items = $wpdb->prefix . 'learnpress_user_items';
$inserted = $wpdb->insert( $lp_items, array( 'user_id' => $course_student_id, 'item_id' => $course_id, 'item_type' => 'lp_course', 'status' => 'enrolled' ) );
gb_mfa_test_assert( false !== $inserted, 'disposable LearnPress enrollment fixture created' );
gb_mfa_test_assert( $plugin->student_requires_mfa( $course_student_id ), 'LearnPress course enrollment independently requires MFA' );
$course_redirect = $plugin->guard_login_redirect( home_url( '/my-account/' ), home_url( '/my-account/my-courses/' ), get_user_by( 'id', $course_student_id ) );
gb_mfa_test_assert( false !== strpos( $course_redirect, '/mfa-enroll/' ), 'course student cannot land in My Account before MFA enrollment' );
...
gb_mfa_test_assert( false !== strpos( $english, 'I need to install an authenticator app' ), 'English enrollment explains how to obtain an authenticator app' );
gb_mfa_test_assert( false !== strpos( $english, 'Google Authenticator' ) && false !== strpos( $english, 'Microsoft Authenticator' ), 'supported authenticator choices are named' );
gb_mfa_test_assert( false !== strpos( $english, 'apps.apple.com' ) && false !== strpos( $english, 'play.google.com' ), 'official Apple and Google app-store links are available' );
...
gb_mfa_test_assert( false !== strpos( $english, 'enter the code manually' ) && false !== strpos( $english, 'Time based' ), 'same-phone setup has app-specific directed steps' );
...
gb_mfa_test_assert( false !== strpos( $english, 'Setup progress' ) && false !== strpos( $english, 'Authenticator' ), 'English setup progress is exposed accessibly' );
...
gb_mfa_test_assert( false !== strpos( $spanish, 'Necesito instalar una aplicación de autenticación' ), 'Spanish app-install guidance is available' );
gb_mfa_test_assert( false !== strpos( $spanish, 'Ingresar una clave de configuración' ) && false !== strpos( $spanish, 'Otra cuenta' ), 'Spanish app-specific manual-entry instructions are available' );
...
$tabs = $plugin->public_course_tabs( array( 'overview' => array(), 'instructor' => array() ) );
gb_mfa_test_assert( ! isset( $tabs['instructor'] ), 'public LearnPress instructor tab is removed' );
$instructor_html = $plugin->public_instructor_html( '<a href="/instructor/admin/">Admin</a>' );
gb_mfa_test_assert( false === stripos( $instructor_html, 'admin' ) && false === stripos( $instructor_html, 'href' ) && false !== strpos( $instructor_html, 'Gulf Breeze Driving School' ), 'public instructor output contains only the school identity' );
$instructor_components = $plugin->public_instructor_html( array( 'avatar' => 'Admin avatar', 'name' => '<a href="/instructor/admin/">Admin</a>' ) );
gb_mfa_test_assert( is_array( $instructor_components ) && false === stripos( implode( '', $instructor_components ), 'admin' ), 'modern LearnPress instructor component output also removes administrator identity' );
