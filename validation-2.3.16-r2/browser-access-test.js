...
(async () => {
  ...
  const authCookie = JSON.parse(
    fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r2/auth-cookie.json`, 'utf8')
  );
  ...
  const courseId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r2/course-id.txt`, 'utf8').trim();
  const lessonOneId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r2/lesson-one-id.txt`, 'utf8').trim();
  const lessonTwoId = fs.readFileSync(`${process.env.GITHUB_WORKSPACE}/validation-2.3.16-r2/lesson-two-id.txt`, 'utf8').trim();
