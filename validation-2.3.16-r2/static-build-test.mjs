...
const pluginDir = process.env.GB_PLUGIN_DIR || path.join(root, 'build-2.3.16-r2', 'gulf-breeze-core');
...
const workZoneMatch = topicSeven.match(/'adult_en_034'\s*=>\s*<<<'HTML'\n([\s\S]*?)\nHTML,/);
assert(workZoneMatch, 'adult_en_034 hardening source is unavailable.');
const workZoneSourceWords = (workZoneMatch[1].replace(/<[^>]*>/g, ' ').match(/\b[\p{L}\p{N}][\p{L}\p{N}’'-]*\b/gu) || []).length;
assert(285 + workZoneSourceWords >= 900, `adult_en_034 lacks a safe margin above its seven-minute boundary: ${285 + workZoneSourceWords}/840 words.`);
...
console.log('Static Core 2.3.16 revision 2 build checks passed.');
