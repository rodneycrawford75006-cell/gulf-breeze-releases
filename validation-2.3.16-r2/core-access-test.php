...
$validation_dir = $workspace . '/validation-2.3.16-r2';
...
$work_zone_filler = implode( ' ', array_fill( 0, 281, 'workzoneword' ) );
...
foreach ( $lesson_ids as $lesson_index => $resume_lesson_id ) {
	...
	$filler = 'adult_en_032' === $key ? $boundary_filler : ( 'adult_en_034' === $key ? $work_zone_filler : $general_filler );
...
$work_zone_content = $resume_content_after['adult_en_034'];
$work_zone_plain   = wp_strip_all_tags( $work_zone_content );
preg_match_all( "/\\b[\\p{L}\\p{N}][\\p{L}\\p{N}’'-]*\\b/u", $work_zone_plain, $work_zone_words );
$work_zone_count = count( $work_zone_words[0] );
if ( $work_zone_count < 900 || $work_zone_count !== (int) get_post_meta( $lesson_ids[33], '_gb_duration_audit_words', true ) ) {
	throw new RuntimeException( 'Corrected adult_en_034 did not preserve a safe margin above the strict seven-minute duration screen: ' . $work_zone_count . ' words.' );
}
