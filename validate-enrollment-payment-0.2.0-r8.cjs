...
const root = process.env.GB_EP_PLUGIN_DIR || path.resolve(__dirname, 'combined-r8', 'gulf-breeze-enrollment-payment');
...
const checks = [
  ['version r8', /Version:\s*0\.2\.0-dev-r8/.test(php) && /const VERSION = '0\.2\.0-dev-r8'/.test(php)],
  ...
  ['resend includes paid context', php.includes('Open My Courses') && php.includes('course_paid_total') && php.includes('administrator_resend_accepted_by_woocommerce_mailer')],
  ...
  ['existing verified Processing orders receive repeatable completion reconciliation', php.includes('reconcile_existing_paid_course_orders') && php.includes('order_is_ready_for_completion') && php.includes("'status' => 'processing'") && !php.includes('gb_ep_order_completion_sync_version')],
  ['completion reconciliation verifies durable enrollment evidence', php.includes("$order->get_meta( self::META_STUDENT_USER_ID )") && php.includes("get_user_meta( $user_id, '_gb_ep_active_course_'") && php.includes("has_enrolled_or_finished")],
  ['Core blanket 404 hook is replaced', php.includes('replace_core_course_access_gate') && php.includes("remove_action( 'template_redirect', array( $core, 'protect_internal_course_objects' ), 0 )")],
  ['paid enrolled students receive protected course access', php.includes('protect_course_objects_for_entitled_students') && php.includes("'publish' === get_post_status( $course_id )") && php.includes('$this->paid_course_order( $user_id, $course_id )')],
  ['unauthorized registered course objects remain 404 protected', php.includes("$wp_query->set_404()") && php.includes("is_singular( array( 'lp_course', 'lp_lesson', 'lp_quiz' ) )") && php.includes('protected_course_id_for_object')],
  ...
  ['test reset preserves evidence and adds audit note', php.includes('wc_get_orders') && php.includes('PayPal reference') && php.includes('signed agreement') && php.includes('audit evidence were retained')],
  ['marked test account syncs to Core allowlist', php.includes('sync_internal_tester_access') && php.includes("'_internal_tester_user_ids'") && php.includes("get_option( 'gb_course_registry'")],
  ['reset orders detached from student account', php.includes('META_TEST_RESET_DETACHED_USER') && php.includes('$order->set_customer_id( 0 )') && php.includes('student-facing account')],
  ...
  ['no stale development version', !/0\.2\.0-dev-r[1-7]/.test(php + readme)],
];
